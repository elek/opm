		...
		for _, repo := range repoList {
			...
			for _, prName := range prList {
				...
				err = output.Write(Contribution{
					...
					Repo:       repoName,
					Type:       "PR_CREATED",
					Date:       json.ME(time.RFC3339, pr, "createdAt"),
				...
				for _, comment := range json.L(json.M(pr, "comments", "nodes")) {
					...
					err = output.Write(Contribution{
						...
						Repo:       repoName,
						Type:       "PR_COMMENTED",
						Date:       json.ME(time.RFC3339, comment, "createdAt"),
						Identifier: json.MNS(pr, "number"),
				...
				for _, review := range json.L(json.M(pr, "reviews", "nodes")) {
					...
					err = output.Write(Contribution{
						...
						Date:       json.ME(time.RFC3339, review, "updatedAt"),
						Identifier: json.MNS(pr, "number"),
				...
				if json.MB(pr, "merged") {
					...
					err = output.Write(Contribution{
						...
						Repo:       repoName,
						Type:       "PR_MERGED",
						Date:       json.ME(time.RFC3339, pr, "mergedAt"),
						Identifier: json.MNS(pr, "number"),
